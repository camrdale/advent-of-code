...
from aoc.log import log, INFO, DEBUG
...
class Towels:
    ...
    def __init__(self, input: str):
        self.towels = frozenset(input.split(', '))
    ...
    def build_design(self, design: str) -> int:
        ...
        for l in range(self.max_length, 0, -1):
            if len(design) >= l and design[:l] in self.towels:
                log(DEBUG, 'Trying', design[:l], 'remaining:', design[l:])
                num_possibilities += self.build_design(design[l:])

        log(INFO, f'{num_possibilities} for design {design}')
