...
@functools.cache
def conway_element_look_and_say(element: str, remaining: int) -> int:
    if remaining == 0:
        return len(ATOMIC_SEQUENCES[element])
    return sum(conway_element_look_and_say(decayed_element, remaining - 1)
               for decayed_element in ATOMIC_DECAYS[element])


def conway_look_and_say(s: str, remaining: int) -> int:
    conway_element_look_and_say.cache_clear()
    for name, sequence in ATOMIC_SEQUENCES.items():
        if sequence == s:
            return conway_element_look_and_say(name, remaining)
    raise ValueError(f'Failed to find Conway atomic element in: {s}')
