...
class Part1(Part):
    def run(self, parser: InputParser) -> int:
        ...
        for equality in equalities:
            pushed_digit_value = max(equality.range_for_pushed_digit())
        ...
        log.log(log.RESULT, f'The largest model number accepted by MONAD: {"".join(map(str, model_number))}')
        return int("".join(map(str, model_number)))


part = Part1()

part.add_result(98491959997994)
